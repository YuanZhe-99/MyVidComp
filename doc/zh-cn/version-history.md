...
- `2026-09-06`：为 Android 安装包配上自己的启动图标。此前它一直沿用 Flutter 的默认图标，因为图标
  设置里写了 Windows 和 macOS，却始终没写 Android。图标同时以 Android 从第 8 版起就想要的、与形状
  无关的形式提供：画面缩进后置于白色底上，这样把图标画成圆形的启动器裁掉的是背景，而不是画面。
